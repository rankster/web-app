...
class EloLite implements Ranker
{
    public function update(Rank $winner, Rank $loser, $isDraw = false)
    {
        $rating = new self();
        if ($isDraw) {
            $rating->set($winner->rank, $loser->rank, 0.5, 0.5);
        } else {
            $rating->set($winner->rank, $loser->rank, 1, 0);
        }

        $results = $rating->getNewRatings();

        $winner->rank = $results['a'];
        $loser->rank = $results['b'];
    }
     ...
     */
    public function set($ratingA, $ratingB, $scoreA, $scoreB)
    {
