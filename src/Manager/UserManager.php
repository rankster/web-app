...
class UserManager
{
    public static function deleteUser(User $user)
    {
        ...
        Rank::statement()->delete()
            ->where('? = ?', Rank::columns()->userId, $user->id)
            ->query()->execute();

        RankHistory::statement()->delete()
            ->where('? = ?', RankHistory::columns()->userId, $user->id)
            ->query()->execute();

        Match::statement()->delete()
            ->where('? IN (?, ?)', $user->id, Match::columns()->user1Id, Match::columns()->user2Id)
            ->query()->execute();
