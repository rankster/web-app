            ...
            <?php foreach (\Rankster\Entity\Rank::getRanks($game->id) as $i => $rank) {
                ...
                <tr>
                    ...
                    <td><img class="img-circle"
                             src="<?php echo \Rankster\Entity\User::pathToUrl($rank['picture_path']); ?>"/></td>
                    <td class="col-lg-3">
                        <a href="/user/details?user_id=<?= $user->id ?>"><?php echo $user->name; ?></a><br/>
                        <a href="/user/match-history?user_id=<?= $user->id ?>&game_id=<?=$game->id?>"><?= $r->matches ?> matches</a> played
                    </td>
                    ...
                    <td style="vertical-align: middle">
                        <div title="Submit score" class="btn btn-xs btn-danger waves-effect waves-light m-b-5"
                             onclick='Rankster.gameReplayDialog(<?= json_encode($game->toArray()) ?>, <?= json_encode($user->toArray()) ?>)'>
